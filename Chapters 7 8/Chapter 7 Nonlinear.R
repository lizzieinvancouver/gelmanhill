...
for (s in 1:n.sims){ # for each parameter value from the model
  pred <- X.tilde %*% coef(sim.88)[s,] # matrix multiplication
  ok <- !is.na(pred)
  y.tilde[s,ok] <- rnorm (sum(ok), pred[ok], sim.88@sigma[s]) # generate predictions for all the contested elections, using the matrix of predictions from the model, and the sd from the model, drawing from a normal distribution.
}

summary(y.tilde) # now for each election in 1990, there are 1000 predictions. 

## Predictive simulation for a nonlinear function of new data

hist(dems.tilde <- rowSums (y.tilde > .5, na.rm=T), main = "Histogram of number of elections \n predicted to go Dem in 1990")
