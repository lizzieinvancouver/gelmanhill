### Extracted from analysesataglance.R ###
### In: vinmisc/vintages ###
...
setwd("~/Documents/git/teaching/gelmanhill/ordinal/lizzie_example")
...
# vintage data
allstars4reg <- read.csv("output/allstars4reg.csv", header=TRUE)
# climate data from Ben
temp <- read.csv("climate/seas_temp_MJJ.onedeg.csv",
    header=TRUE)
colnames(temp)[1] <- "year"
prec <- read.csv("climate/seas_prec_MJJ.onedeg.csv",
    header=TRUE)
colnames(prec)[1] <- "year"
pdsi <- read.csv("climate/seas_pdsi_MJJ.onedeg.csv",
    header=TRUE)
...
###############################################################
## HERE HERE! This is perhaps most useful to review as class ##
###############################################################
...
################################################
## CLM models ... and formatting for Sweave ####
################################################
