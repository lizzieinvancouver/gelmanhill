...
for (i in 1:20)
  curve (coef(sim.logmodel.1)[i,1] + coef(sim.logmodel.1)[i,2]*x, lwd=.5, col="gray", add=TRUE)
...
for (i in 1:20)
  curve (exp(coef(sim.logmodel.1)[i,1] + coef(sim.logmodel.1)[i,2]*x), lwd=.5, col="gray", add=TRUE)
